...
use elsa::FrozenVec;
use janus_io::upload::{MultiUploader, UploadError};
...
impl PushTask {
    ...
    async fn _process_queue(&mut self) -> Result<(), Error> {
        ...
        let mut uploader = self.vfs.remote_storage().prepare_multi_upload()?;
        ...
        loop {
            ...
            tx.commit().await?;
            uploader = self.vfs.remote_storage().prepare_multi_upload()?;
        }
    }
