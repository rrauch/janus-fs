...
impl Vfs {
    pub async fn delete_fs(
        ...
        let mut deletable_objects = remote_storage
            .list_objects()
...
mod tests {
    ...
    async fn delete_all() -> anyhow::Result<()> {
        ...
        let mut deleted = Vfs::delete_fs(&id1, &remote_storage).await?;
        deleted += Vfs::delete_fs(&id2, &remote_storage).await?;
        deleted += Vfs::delete_fs(&id3, &remote_storage).await?;
        assert_eq!(deleted, 9);

        assert!(Vfs::scan(&remote_storage).await?.is_empty());
        Ok(())
    }
    ...
    async fn nonexistent() -> anyhow::Result<()> {
        ...
        assert_eq!(Vfs::delete_fs(&vfs_id, &remote_storage).await?, 0);
        Ok(())
    }
}
